...
export function PresidencyCard({ rank, game, legacy, reason, archetype }: PresidencyCardProps) {
  return (
    <Link
      href={`/game/${game.id}`}
      className="block rounded-sm border border-[var(--color-border)] bg-[var(--color-surface)] px-5 py-4 backdrop-blur-sm transition-colors hover:border-[var(--color-border-strong)]"
    >
